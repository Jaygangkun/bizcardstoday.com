...
flog('card', $card);
	...
	if($alerter=="")
	{
		$statement="SELECT ID, Filename FROM Finished_Cards WHERE ID=" . $card['ID'] . " and Template='" . $template . "'";
		$sql->QueryRow($statement);
...
flog('rows', $a);
		...
		{
			if($_POST['Line_' . $i]!="")
				$statement .= ", Line_" . $i . "=\"" . stripslashes(str_replace("&#39;", "'", str_replace("\"", "&quot;", $_POST['Line_' . $i]))) . "\""; //HTTP_POST_VARS
			else{
				$statement .= ", Line_" . $i . "=\"" . stripslashes(str_replace("&#39;", "'", str_replace("\"", "&quot;", $card['Line_' . $i]))) . "\"";
				$HTTP_POST_VARS['Line_' . $i]=$card['Line_' . $i];
			}
			//echo "<!--$i: " . stripslashes(str_replace("&#39;", "'", str_replace("\"", "&quot;", $HTTP_POST_VARS['Line_' . $i]))) . "-->\n";
		}
		//echo "<!--" . $statement . "-->\n";
		if($card['Action']=="New")
		{
		...
		//$statement .=", Quantity='" . $HTTP_POST_VARS['Quantity'] . "', Quality='" . $HTTP_POST_VARS['card_quality'] . "', Notes=\"" . addslashes($HTTP_POST_VARS['Notes']) . "\", Filename=\"" . $file . "\", po='" . $HTTP_POST_VARS['po'] . "', company='" . $HTTP_POST_VARS['company'] . "', name='" . $HTTP_POST_VARS['name'] . "', address1='" . $HTTP_POST_VARS['address1'] . "', address2='" . $HTTP_POST_VARS['address2'] . "', city='" . $HTTP_POST_VARS['city'] . "', state='" . $HTTP_POST_VARS['state'] . "', zip='" . $HTTP_POST_VARS['zip'] . "', speed='" . $HTTP_POST_VARS['speed'] . "', ship='" . $HTTP_POST_VARS['ship'] . "', paper='" . $Paper . "', Ink='" . $Ink . "'";
		if($_POST['hold']=='no')
		{
			if($target!="")
...
if(DEBUGSW) 
{
	...
	while($j<=$numlines)
	{
		$card['Line_' . $j] = str_replace('&reg;', '®', stripslashes(str_replace('*', '·', htmlentities(str_replace("&quot;", "\"", $card['Line_' . $j]), ENT_NOQUOTES))));
		$j++;
	}
	/*
...
var messege = "<BR><BR><BR><BR><BR><BR><center><p><font color='#000000' size='2'>Processing Order. Please be patient, this process can take around 2 minutes </font></p><BR><img src=Untitled-6.gif></center><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR>";
	...
	<div id="uploadmes"><img src=Untitled-6.gif width=1 height=1></div>
		...
		<tr>
			<td>
				...
				<embed src="<?php echo $file ?>"  <?php echo $dimension; ?> type="image/svg+xml" pluginspage="http://www.adobe.com/svg/viewer/install/auto/"></td></tr></table>
			...
			<?
				if($card_quality=='e')
				{
					...
					echo "\t<font size=+3>BizCards now come in 2 flavors <br> <b>Standard &amp; Premium</font><br><input type=button value=\"More Info\" onclick=\"window.open ('picinfo.php?pic_width=" . $card['Pic_Width'] . "&pic_height=" . $card['Pic_Height'] . "&msg=2', 'view', 'width=" . $width . ",height=" . $height . "');\"></b>\n";
				}
			?>
			</td>
		</tr>
...
<input type='hidden' name='Line_1' value='<?php echo($_POST['Line_1']); ?>'>
			...
			<tr>
				<td colspan=2 valign=top bgcolor=#D1D1D1><table border=0 cellspacing=0 cellpadding=0>
					<tr>
						<td colspan=4 valign=top><b>How Many Cards</b> Would You Like To Order</td>
					</tr>
					<tr>
						<?
							if($card_quality=="s")
							{
								...
								if($per250>0) //If a price has been defined for 250, then allow them to order 250
								{
									...
									if($card['Quantity']=='250' || (($card['Quantity']=="0" || $card['Quantity']=="") && $default_quantity=='250'))
										echo "checked";
									echo ">250</td>\n";
								}
								if($per500>0) //If a price has been defined for 500, then allow them to order 500
								{
									...
									if($card['Quantity']=='500' || (($card['Quantity']=="0" || $card['Quantity']=="") && $default_quantity=='500'))
										echo "checked";
									echo ">500</td>\n";
								}
								if($per1000>0) //If a price has been defined for 1000, then allow them to order 1000
								{
									...
									if($card['Quantity']=='1000' || (($card['Quantity']=="0" || $card['Quantity']=="") && $default_quantity=='1000'))
										echo "checked";
									echo ">1000</td>\n";
								}
								if($per2000>0) //If a price has been defined for 2000, then allow them to order 2000
								{
									...
									if($card['Quantity']=='2000' || (($card['Quantity']=="0" || $card['Quantity']=="") && $default_quantity=='2000'))
										echo "checked";
									echo ">2000</td>\n";
								}
							...
							{
								if($per250_premium>0 || $per250>0) //If a price has been defined for 250, then allow them to order 250
								{
									echo "<td><input type=radio value=250 name=Quantity ";
									if($card['Quantity']=='250' || (($card['Quantity']=="0" || $card['Quantity']=="") && $default_quantity=='250'))
										echo "checked";
							...
							if($card_quality=='e')
							{
								echo "\t\t\t\t\t</tr><tr>\n\t\t\t\t\t\t<td colspan=4><span id=Blinker>Card Quality </span> (<a href=# onclick=\"window.open ('picinfo.php?pic_width=" . $card['Pic_Width'] . "&pic_height=" . $card['Pic_Height'] . "&msg=2', 'view', 'width=$width,height=$height');\">What's This?</a>)&nbsp;&nbsp;&nbsp;<input type=radio name=card_quality value='s' ";
			...
			<tr>
				<td colspan=2 bgcolor=#D1D1D1>In a mad rush? <input type=checkbox name=speed value='y'> Yes, I want my cards ready within 24 hours (Rush Charges Apply)<br>How would you like your order shipped: <input type=radio name="ship" value='s' <?php if($card['ship']=='s' || $card['ship']=='') echo "checked"; ?>> UPS Ground <input type=radio name="ship" value='o' <?php if($card['ship']=='o') echo "checked"; ?>> Next Day <input type=radio name="ship" value='2' <?php if($card['ship']=='2') echo "checked"; ?>> 2 Day Air </td>
			</tr>
			<tr><?
			...
			?>
				...
				<td colspan=2 bgcolor=#D1D1D1><table border=0 cellspacing=2 cellpadding=0>
					<tr>
						...
						<td>Company:</td><td><input type=text name=company value="<?php echo $card['company']; ?>" size=35></td>
					</tr><tr>
						<td>Name:</td><td><input type=text name=name value="<?php echo $card['name']; ?>" size=35></td>
					</tr><tr>
						<td>Address 1:</td><td><input type=text name=address1 value="<?php echo $card['address1']; ?>" size=35></td>
					</tr><tr>
						<td>Address 2:</td><td><input type=text name=address2 value="<?php echo $card['address2']; ?>" size=35></td>
					</tr><tr>
						<td colspan=2>City: <input type=text name=city value="<?php echo $card['city']; ?>" size=20> State: <input type=text name=state value='<?php echo $card['state'] ?>' size=3 maxlength=2> Zip:<input type=text name=zip value='<?php echo $card['zip'] ?>' size=10 maxlength=10></td>
					</tr></tr>
				...
				<?php
					...
					{
						echo "<td><table border=0 cellspacing=0 cellpadding=0><tr><td><input type=submit value='Approve Order' name=button onclick='return OnButton2();'></td><td><input type=submit value='Return to Menu' name=button onclick='return OnButton5();'></td></tr></table></td>\n";
					}else{		//Regular Order placement
						echo "<td><table border=0 cellspacing=0 cellpadding=0><tr><td colspan=3><b>Place Order</b> will save the card and submit an order for printing</td></tr><tr><td>";
						?>
<!--						<form id="upload_file" action="place_order.php" METHOD="post">
						<span id="btn"></span>-->
<!--						onClick="disable(); submit();"-->
<!--						</form>-->
<!--			<input type="submit" value="Place Order"  tabindex="2">--><input type="hidden" name="fileName" value="<?php echo($file); ?>">
